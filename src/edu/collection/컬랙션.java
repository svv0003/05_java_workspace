...
컬랙션의 대표적인 프레임워크
List, Set, Map 인터페이스가 존재한다.

List 인터페이스  : List를 기반드로 만들어진 ArrayList 객체를 주로 활용하고, 매우 중요하다.



공통점  : 여러 데이터를 한 번에 저장 가능하고, 크기가 자동으로 변한다.
          반복으로 순회 가능하고, 목록에 들어올 수 있는 자료형을 제한하거나 제한하지 않는다.
List    : 순서대로 나열, 중복 허용, 순서 보장, 인덱스 접근 가능
          -> Java에서 주로 사용한다.

Set     : 중복 불가, 순서는 종류에 따라 다름, 인덱스 접근 불가
          (주머니에 순서 없이 넣고, 담배 두 개 같이 넣지 않는 것과 같다.)
          -> 많이 사용하지 않는다.

Map     : Key - Value 형태
          인덱스 번호 대신 개발자가 설정한 key로 데이터를 조회하거나 수정 가능하다.
          key 명칭 중복 불가, 값의 중복은 가능, 순서는 값에 따라 다름, key 명칭으로 접근 가능, 인덱스 접근 불가
          -> Java, JS에서 주로 사용한다.
 */
