...
폴더명 파일명.확장자명 경로를 사용할 수 있도록 설정된 기능
    ...
    /*
Paths.get()     1개 이상의 문자열 작성한다.
                ,를 이용하여 여러 경로 요소를 조합 가능하다.

Path.of()       1개 이상의 문자열 작성한다.
                더 간결한 문법 사용한다.
                Java 11부터 사용 권장한다.

Path.getParent()         파일명 앞 모든 파일경로를 가져오는 기능
Path.getFileName()       파일경로 외 파일명.확장자명만 가져오는 기능
Path.toAbsolutePath()    절대경로 반환
Path.getNameCount()      파일에 도달하기까지 /의 개수 (= 경로 개수) 반환
int nameCount            path.getNameCount()로 파일명 전까지의 / 개수로 폴더 개수 가져온다.
Path.getName(0)          index 상 C: 반환한다.
Path.getName(1)          index 상 파일경로1 반환한다.



Files 클래스 주요 메서드
Files.copy (Path.of ("원본파일경로/원본파일명.확장자명"), Path.of ("복사파일경로/복사파일명.확장자명"));
...
Files.deleteIfExists (Path.of ("경로/파일명.확장자명"));     => 삭제할 파일이 존재하지 않아도 예외 발생하지 않는다.
                                                                파일이 존재하는데 다른 곳에서 사용할 때는 예외 발생한다.

Files.createDirectories(path.getParent());
Files.
      createDirectories()                         파일명 앞 모든 파일경로가 없으면 생성하는 기능
                         Path.getParent()         파일명 앞 모든 파일경로를 가져오는 기능


Streaming               네트워크를 통해 데이터 (오디오나 비디오 같은 미디어)를 실시간으로 받아오는 기법
Stream                  데이터의 연속적인 흐름을 나타내는 개념
                        파일, 네트워크, 메모리 등에서 데이터를 순차적으로 읽고 쓸 때 사용한다.
     */


    public void method99() {

        Path path = Path.of("Users/파일경로1/파일경로2/파일경로3/파일명.txt");
