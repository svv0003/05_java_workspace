public class PrintEx1 {
    public static void main(String[] args) {
        ...
        * System.out.print();           - () 내의 값을 출력한다. (출력 후 줄바꿈 X)
        * System.out.println();         - () 내의 값을 출력한다. (출력 후 줄바꿈 O)
        ...
        *                           음수 (-)  - 왼쪽 정렬
        *
        *  %숫자d     : 정수가 출력된 칸을 숫자만큼 확보한다.
        *  ex) %-6d  : ㅁㅁㅁㅁㅁㅁ 숫자 자리 확보
        *
        *  %.숫자f    : 소수점 아래 몇 번째 자리까지 표시할 지 지정한다.
        *               (지정된 자리 아래에서 반올림 처리)
        *
        *  - 두 번째 매개 변수
        ...
        * */
        ...
        String name = "박세원";    // 세미콜론(;)은 마침표와 같은 의미로 코드 작성의 종료를 의미한다.
        int age = 20;
        double height = 190.35;
        char gender = 'M';
        boolean javaStudent = true;

        // 박세원은 20세 M으로 키는 190.35이고, 자바 공부 여부 : true
        System.out.printf("%s은 %d세 %c성으로 키는 %.1f cm이고, 자바 공부 여부 : %b", name, age, gender, height, javaStudent);

        // 자바는 중괄호{} 내부에 작성하는 것을 매우 중요하게 여기므로 반드시 class 내부에 작성한다.

        System.out.println("\n============================");

        // \n === 개행 문자 (줄바꿈)
        System.out.printf("%8s %-6s %7d원 %s \n", "20250825", "홍길동", 10000, "입금");
        System.out.printf("%8s %-6s %7d원 %s \n", "20250825", "아파트 관리비", 120000, "입금");
        System.out.printf("%8s %-6s %7d원 %s \n", "20250825", "식자재마트", 25000, "입금");
